document.addEventListener('DOMContentLoaded', function() {
    ...
    const uiText = {
        ...
        hi: {
            ...
            heroTitle: 'एग्रोखेती', heroSubtitle: 'पी.एस.ए. कंटेंट डेस्क द्वारा', heroDate: '15 सितम्बर 2025', heroBtn: 'किसानों की मदद के लिए',
    ...
    function updateWebsiteLabels(lang) {
        ...
        for (const key in navElements) {
            if (navElements[key]) {
                if (key === 'search') {
                    navElements[key].placeholder = labels.search;
                } else if (key.startsWith('footer')) {
                    const index = navElements.footerLinks.indexOf(navElements[key]);
                    if (index !== -1) navElements[key].textContent = labels.footerLinks[index];
                } else {
                    navElements[key].textContent = labels[key];
                }
            }
        }

        // Handle footer links specially to avoid errors
        if (labels.footerLinks) {
            navElements.footerLinks.forEach((el, i) => {
                if (el) el.textContent = labels.footerLinks[i] || el.textContent;
            });
        }
    }
